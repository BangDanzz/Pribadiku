...
const makemeazombie = require('./zombie');
...
router.get("/api/ai/zombie", checkApiKeys, async (req, res) => {
    ...
    try {
        const zombieMaker = new makemeazombie();
        const transformedImage = await zombieMaker.transform({ photo: url });
        const imageBuffer = Buffer.from(transformedImage, "base64");

        res.setHeader("Content-Type", "image/jpeg");
        res.setHeader("Content-Disposition", 'inline; filename="zombie.jpg"');
        res.end(imageBuffer);
    } catch (err) {
        console.error("Zombie generator failed:", err.message);
